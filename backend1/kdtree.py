...
def KDTREE(file_name, k, working_dir, block_dictionary):
    ...
    if not os.path.exists(imgPath):
        ...
        if len(face_enc) == 0:
            ...
            existsFile = os.path.isfile(working_dir+'/KD-TREE.csv')
            ...
            if not existsFile:
                ...
                face_df["img"] = img_df
                face_df.to_csv(working_dir+'/KD-TREE.csv',index=False, encoding='utf-8')    
                ...
                face_df = pd.read_csv(working_dir+'/KD-TREE.csv')
                face_df.reset_index(drop=True, inplace=True)
                tree_instance = KDTree(face_df.iloc[:, 0:-1])
                distances, indices = tree_instance.query(face_enc, k)
                matches = []
                ...
                time_elapsed = str(round((end_time - start_time) * 1000))
                print("Range_search took " + time_elapsed + " ms.")
